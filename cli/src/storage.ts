...
interface CacheData {
  tracks: { [workspaceId: string]: any[] };
  timestamps: { [workspaceId: string]: number };
  metadata: { [trackId: string]: any };
  deviceId?: string;
  authToken?: string;
}
...
export class Storage {
  ...
  cacheTracks(workspaceId: string, tracks: any[]): void {
  ...
  cacheMetadata(trackId: string, metadata: any): void {
    this.cache.metadata[trackId] = metadata;
    this.saveCache();
  }

  getCachedMetadata(trackId: string): any | null {
