...
This command reads a JSON file generated by the converter (`--image-list`) and downloads each artwork entry. The file should be an array of objects with `clipId` and `thumbnail` fields.
...
**Options:**
- `-l, --list <file>` - JSON list file (required)
- `-t, --token <token>` - Authentication token (optional)
- `-b, --browser <url>` - Browser URL for token extraction (optional)
- `-o, --output <dir>` - Output directory (default: ./downloads)
- `--delay <ms>` - Delay between downloads in ms (default: 1000)
